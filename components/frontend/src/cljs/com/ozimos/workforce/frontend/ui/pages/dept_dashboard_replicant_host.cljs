(ns com.ozimos.workforce.frontend.ui.pages.dept-dashboard-replicant-host
  "Host for Replicant DeptDashboard."
  (:require
   [com.fulcrologic.fulcro.components :as comp :refer [defsc]]
   [com.fulcrologic.fulcro.dom :as dom]
   [com.ozimos.workforce.frontend.replicant-bridge :as bridge]
   [com.ozimos.workforce.frontend.ui.pages.dept-dashboard-replicant :as cr]
   [goog.dom :as gdom]))

(defsc DeptDashboardReplicantHost [this _props]
   ...
   (fn [this]
     (let [app (comp/any->app this)
           ...
           {::cr/set-unit-id (fn [ev _field] (let [v (some-> ev :replicant/js-event .-target .-value)] (comp/transact! app [(cr/set-unit-id {:value (or v "")})])))
            ::cr/load (fn [_ev id] (comp/transact! app [(cr/set-loading {:v true})]))}]
       (when node (bridge/install-replicant-root! app cr/DeptDashboardReplicant node handlers))))}
