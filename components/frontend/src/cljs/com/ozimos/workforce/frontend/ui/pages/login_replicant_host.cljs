(ns com.ozimos.workforce.frontend.ui.pages.login-replicant-host
  ...
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.login-replicant :as lr]
   [goog.dom :as gdom]))
     ...
     (let [app-inst (comp/any->app this)
       ...
       (when node
         (let [handlers {::lr/set-identifier (fn [ev]
           ...
           (bridge/install-replicant-root! app-inst lr/LoginReplicant node handlers)))))
