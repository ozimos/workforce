(ns com.ozimos.workforce.frontend.ui.pages.org-chart.host
  "Fulcro host for the Replicant OrgChart page.

   Mounts the pure Replicant view (`org-chart/OrgChart`)
   into a dedicated DOM node via `bridge/install-replicant-root!`,
...
(defsc OrgChartHost [_this _props]
  {:query         [:loading :error :active-org :units :hierarchy :search-term :collapsed-nodes]
   ...
   (fn [this]
     (let [app-inst   (comp/any->app this)
           ...
           node       (gdom/getElement "org-chart")]
       ...
       (when node
         (let [handlers
               ...
               {::cr/toggle-collapse   (fn [_ id]   (comp/transact! app-inst [(cr/toggle-collapse {:id id})]))
                ...
                ::cr/noop              (fn [& _]    nil)}]
           (bridge/install-replicant-root! app-inst cr/OrgChart node handlers)))))
   :componentDidUpdate
   (fn [_this _prev _snapshot]
     ;; Replicant bridge's add-watch handles re-renders automatically.
     nil)}
  (dom/div {:id "org-chart-host" :className "min-h-full"}
    (dom/div {:id "org-chart"} "Loading Replicant OrgChart…")
