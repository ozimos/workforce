(ns com.ozimos.workforce.frontend.ui.pages.reset-password.host
  ...
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.reset-password :as rpr]
   [goog.dom :as gdom]))
...
(defsc ResetPasswordHost [_this _props]
     ...
     (let [app-inst (comp/any->app this)
       ...
       (when node
         (let [handlers {::rpr/set-password (fn [ev]
           ...
           (bridge/install-replicant-root! app-inst rpr/ResetPassword node handlers)))))
