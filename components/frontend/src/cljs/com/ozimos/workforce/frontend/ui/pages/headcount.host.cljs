(ns com.ozimos.workforce.frontend.ui.pages.headcount.host
  ...
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.headcount :as cr]
   [goog.dom :as gdom]))

(defsc HeadcountHost [_this _props]
  {:query [:loading :error :active-org :pending-approvals :submitting :msg
   ...
   (fn [this]
     (let [app  (comp/any->app this)
       ...
       (when node
         (bridge/install-replicant-root! app cr/Headcount node handlers))))}
