(ns com.ozimos.workforce.frontend.ui.pages.create-org.host
  ...
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.create-org :as cor]
   [goog.dom :as gdom]))
...
(defsc CreateOrgHost [_this _props]
     ...
     (let [app-inst (comp/any->app this)
       ...
       (when node
         (let [handlers {::cor/set-name (fn [ev]
           ...
           (bridge/install-replicant-root! app-inst cor/CreateOrg node handlers)))))
