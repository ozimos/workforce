(ns com.ozimos.workforce.frontend.ui.pages.forgot-password-replicant-host
  ...
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.forgot-password-replicant :as fpr]
   [goog.dom :as gdom]))
     ...
     (let [app-inst (comp/any->app this)
       ...
       (when node
         (let [handlers {::fpr/set-email (fn [ev]
           ...
           (bridge/install-replicant-root! app-inst fpr/ForgotPasswordReplicant node handlers)))))
