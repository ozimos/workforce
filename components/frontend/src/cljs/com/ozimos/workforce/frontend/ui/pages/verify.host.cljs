(ns com.ozimos.workforce.frontend.ui.pages.verify.host
  ...
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.verify :as vr]
   [goog.dom :as gdom]))
     ...
     (let [app-inst (comp/any->app this)
       ...
       (when node
         (let [handlers {::vr/navigate (fn [_ path] (set! js/window.location.href path))}]
           (bridge/install-replicant-root! app-inst vr/Verify node handlers)))
