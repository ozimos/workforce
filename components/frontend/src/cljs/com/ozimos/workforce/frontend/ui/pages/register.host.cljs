(ns com.ozimos.workforce.frontend.ui.pages.register.host
  ...
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.register :as rr]
   [goog.dom :as gdom]))
...
(defsc RegisterHost [_this _props]
     ...
     (let [app-inst (comp/any->app this)
       ...
       (when node
         (let [handlers {::rr/set-email (fn [ev]
           ...
           (bridge/install-replicant-root! app-inst rr/Register node handlers)))))
