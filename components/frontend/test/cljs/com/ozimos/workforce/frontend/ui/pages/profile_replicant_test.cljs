...
(deftest denormalize (testing "denorm" (let [app (app/headless-synchronous-app sut/ProfileReplicant) a (::app/state-atom app)] (swap! a merge (base-props {})) (let [q (:query (meta sut/ProfileReplicant)) t (denorm/db->tree q @a @a) h (sut/ProfileReplicant t)] (is (= "" (:new-username t))) (is (true? (valid? h))) (is (str/includes? (rs/render h) "Profile"))))))
(deftest interaction (testing "set username via transact" (let [app (app/headless-synchronous-app sut/ProfileReplicant) a (::app/state-atom app) q (:query (meta sut/ProfileReplicant))] (swap! a merge (base-props {:new-username ""})) (let [h1 (sut/ProfileReplicant (denorm/db->tree q @a @a))] (is (str/includes? (rs/render h1) "New username")) (comp/transact! app [(sut/set-new-username {:value "bob"})]) (is (= "bob" (:new-username @a))) (let [h2 (sut/ProfileReplicant (denorm/db->tree q @a @a))] (is (str/includes? (rs/render h2) "bob")))))))
(deftest metadata (testing "query ident" (is (= [:new-username :error-msg :success-msg :loading :mfa-stage :mfa-secret :mfa-qr-url :mfa-backup-codes :totp-code] (:query (meta sut/ProfileReplicant)))) (is (= :profile-replicant/root (:ident (meta sut/ProfileReplicant))))))
