...
(deftest headless-denormalize-and-mutation
  (testing "in-memory interaction cycle"
    (let [app-inst (app/headless-synchronous-app sut/JoinOrgReplicant)
          state-atom (::app/state-atom app-inst)
          query (:query (meta sut/JoinOrgReplicant))]
      (swap! state-atom merge (base-props {}))
      (let [hiccup-1 (sut/JoinOrgReplicant (denorm/db->tree query @state-atom @state-atom))]
        ...
        (let [hiccup-2 (sut/JoinOrgReplicant (denorm/db->tree query @state-atom @state-atom))]
