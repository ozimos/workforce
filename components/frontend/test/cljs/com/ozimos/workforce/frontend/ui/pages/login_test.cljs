(ns com.ozimos.workforce.frontend.ui.pages.login-test
  (:require
   [cljs.test :refer [deftest is testing]]
   [clojure.string :as str]
   [com.fulcrologic.fulcro.algorithms.denormalize :as denorm]
   [com.fulcrologic.fulcro.application :as app]
   [com.ozimos.workforce.frontend.ui.pages.login :as sut]
...
(deftest standard-login-render
  (testing "renders identifier and password fields"
    (let [hiccup (sut/Login (base-props {}))
          html (rs/render hiccup)
          input-ev (find-event-in-hiccup hiccup #(= (first %) :com.ozimos.workforce.frontend.ui.pages.login/set-identifier))]
      (is (str/includes? html "Sign in to your account"))
      (is (str/includes? html "Email or username"))
      (is (str/includes? html "Password"))
      (is (= [:com.ozimos.workforce.frontend.ui.pages.login/set-identifier] input-ev))
...
(deftest mfa-challenge-render
  (testing "renders 2FA code input when mfa-required"
    (let [hiccup (sut/Login (base-props {:mfa-required true :mfa-token "tok123"}))
          html (rs/render hiccup)]
...
(deftest error-message-render
  (testing "renders error banner"
    (let [html (rs/render (sut/Login (base-props {:error-msg "Invalid credentials"})))]
