(ns com.ozimos.workforce.frontend.ui.pages.headcount-test
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.headcount :as sut]
...
(defn- hiccup->html [props] (rs/render (sut/Headcount props)))
(defn- hiccup-tree [props] (sut/Headcount props))
...
(deftest action-events-are-pure
  (testing "approve event is pure vector [::approve id]"
    (let [hiccup (hiccup-tree (base-props {}))
          ev (find-event-in-hiccup hiccup #(= (first %) :com.ozimos.workforce.frontend.ui.pages.headcount/approve))]
      (is (= [:com.ozimos.workforce.frontend.ui.pages.headcount/approve "req-1"] ev))))
  (testing "reject event is pure"
    (let [hiccup (hiccup-tree (base-props {}))
          ev (find-event-in-hiccup hiccup #(= (first %) :com.ozimos.workforce.frontend.ui.pages.headcount/reject))]
      (is (= [:com.ozimos.workforce.frontend.ui.pages.headcount/reject "req-1"] ev))))
  (testing "create event"
    (let [hiccup (hiccup-tree (base-props {}))
          ev (find-event-in-hiccup hiccup #(= (first %) :com.ozimos.workforce.frontend.ui.pages.headcount/create))]
      (is (= [:com.ozimos.workforce.frontend.ui.pages.headcount/create] ev))))
  (testing "events are vectors not fns"
    (let [ev (find-event-in-hiccup (hiccup-tree (base-props {})) #(= (first %) :com.ozimos.workforce.frontend.ui.pages.headcount/approve))]
...
(deftest defrc-metadata
  (testing "query and ident"
    (let [q (:query (meta sut/Headcount))]
      (is (vector? q))
      (is (some #{:loading} q))
      (is (some #(and (map? %) (contains? % :pending-approvals)) q)))
    (is (= :headcount/root (:ident (meta sut/Headcount))))))
...
(deftest pure-state-transitions
  ...
  (testing "plain atom parity with transact!"
    (let [plain-atom (atom (base-props {}))
          app-inst (app/headless-synchronous-app sut/Headcount)
...
(deftest headless-denormalize
  (testing "denormalizes from Fulcro DB"
    (let [app-inst (app/headless-synchronous-app sut/Headcount)
          state-atom (::app/state-atom app-inst)]
      (swap! state-atom merge (base-props {}))
      (let [query (:query (meta sut/Headcount))
            tree (denorm/db->tree query @state-atom @state-atom)
            hiccup (sut/Headcount tree)]
...
(deftest headless-interaction-cycle
  (testing "approve click updates DB and re-render"
    (let [app-inst (app/headless-synchronous-app sut/Headcount)
          state-atom (::app/state-atom app-inst)
          query (:query (meta sut/Headcount))]
      ...
      (let [hiccup-1 (sut/Headcount (denorm/db->tree query @state-atom @state-atom))]
        (is (str/includes? (rs/render hiccup-1) "T"))
        (let [ev (find-event-in-hiccup hiccup-1 #(= (first %) :com.ozimos.workforce.frontend.ui.pages.headcount/approve))]
          (is (= [:com.ozimos.workforce.frontend.ui.pages.headcount/approve "r1"] ev))
          (comp/transact! app-inst [(sut/set-msg {:msg "Approve r1"})])
          (is (= "Approve r1" (:msg @state-atom)))
          (let [hiccup-2 (sut/Headcount (denorm/db->tree query @state-atom @state-atom))]
