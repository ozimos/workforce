(ns com.ozimos.workforce.frontend.ui.pages.reset-password-replicant-test
  (:require
   [cljs.test :refer [deftest is testing]]
   [clojure.string :as str]
   [com.fulcrologic.fulcro.algorithms.denormalize :as denorm]
   [com.fulcrologic.fulcro.application :as app]
   [com.ozimos.workforce.frontend.ui.pages.reset-password-replicant :as sut]
...
(deftest initial-render
  (testing "renders password fields"
    (let [hiccup (sut/ResetPasswordReplicant (base-props {}))
...
(deftest success-state-render
  (testing "renders success screen"
    (let [hiccup (sut/ResetPasswordReplicant (base-props {:success true}))
          html (rs/render hiccup)]
...
(deftest headless-denormalize
  (testing "denormalization in-memory"
    (let [app-inst (app/headless-synchronous-app sut/ResetPasswordReplicant)
          state-atom (::app/state-atom app-inst)
          query (:query (meta sut/ResetPasswordReplicant))]
      (swap! state-atom merge (base-props {:password "p123"}))
      (let [tree (denorm/db->tree query @state-atom @state-atom)
            hiccup (sut/ResetPasswordReplicant tree)]
