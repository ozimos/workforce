...
(deftest metadata (testing "query ident" (let [q (:query (meta sut/OrgDashboardReplicant))] (is (vector? q)) (is (some #{:loading} q)) (is (some #(and (map? %) (contains? % :members)) q))) (is (= :org-dashboard-replicant/root (:ident (meta sut/OrgDashboardReplicant))))))
