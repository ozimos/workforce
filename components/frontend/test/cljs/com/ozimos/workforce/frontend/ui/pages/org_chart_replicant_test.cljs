...
(deftest headless-fulcro-state-denormalization-test
  ...
  ;; ---------------------------------------------------------------------------
  (testing "OrgChartReplicant :query denormalizes correctly from normalized Fulcro DB atom"
    (let [app-inst   (app/headless-synchronous-app sut/OrgChartReplicant)
          state-atom (::app/state-atom app-inst)]
      ;; Seed into the real Fulcro state atom:
      (swap! state-atom merge (base-props {}))
      (let [query  (:query (meta sut/OrgChartReplicant))
            tree   (denorm/db->tree query @state-atom @state-atom)
            hiccup (sut/OrgChartReplicant tree)]
