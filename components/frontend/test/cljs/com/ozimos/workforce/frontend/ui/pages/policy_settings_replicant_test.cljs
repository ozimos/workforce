...
(deftest metadata (testing "query ident" (let [q (:query (meta sut/PolicySettingsReplicant))] (is (vector? q)) (is (some #{:loading} q)) (is (some #(and (map? %) (contains? % :rules)) q))) (is (= :policy-settings-replicant/root (:ident (meta sut/PolicySettingsReplicant))))))
