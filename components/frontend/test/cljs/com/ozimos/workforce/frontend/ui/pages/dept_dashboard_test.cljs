(ns com.ozimos.workforce.frontend.ui.pages.dept-dashboard-test
  (:require
   ...
   [com.ozimos.workforce.frontend.ui.pages.dept-dashboard :as sut]
...
(defn- base-props [overrides]
  (merge {:loading false :error nil :unit-id "eng"
         ...
         overrides))
(defn- hiccup->html [props] (rs/render (sut/DeptDashboard props)))
(defn- hiccup-tree [props] (sut/DeptDashboard props))
...
(deftest switcher
  ...
  (testing "load event is pure"
    (let [ev (find-event (hiccup-tree (base-props {})) #(= (first %) :com.ozimos.workforce.frontend.ui.pages.dept-dashboard/load))]
      (is (vector? ev)) (is (= :com.ozimos.workforce.frontend.ui.pages.dept-dashboard/load (first ev))))))
...
(deftest pure-state
  ...
  (testing "plain atom parity"
    (let [plain (atom (base-props {})) app-inst (app/headless-synchronous-app sut/DeptDashboard) state-atom (::app/state-atom app-inst)]
...
(deftest denormalize-test
  (testing "denormalizes"
    (let [app-inst (app/headless-synchronous-app sut/DeptDashboard) state-atom (::app/state-atom app-inst)]
      (swap! state-atom merge (base-props {}))
      (let [query (:query (meta sut/DeptDashboard)) tree (denorm/db->tree query @state-atom @state-atom) hiccup (sut/DeptDashboard tree)]
        (is (= "eng" (:unit-id tree)))
...
(deftest interaction-cycle
  (testing "set-unit-id via transact"
    (let [app-inst (app/headless-synchronous-app sut/DeptDashboard) state-atom (::app/state-atom app-inst) query (:query (meta sut/DeptDashboard))]
      (swap! state-atom merge (base-props {:unit-id "eng"}))
      (let [hiccup-1 (sut/DeptDashboard (denorm/db->tree query @state-atom @state-atom))]
        (is (str/includes? (rs/render hiccup-1) "eng"))
        (let [ev (find-event hiccup-1 #(= (first %) :com.ozimos.workforce.frontend.ui.pages.dept-dashboard/load))]
          (is (vector? ev))
          (comp/transact! app-inst [(sut/set-unit-id {:value "plat"})])
          (is (= "plat" (:unit-id @state-atom)))
          (let [hiccup-2 (sut/DeptDashboard (denorm/db->tree query @state-atom @state-atom))]
            (is (str/includes? (rs/render hiccup-2) "plat"))))))))

(deftest metadata
  (testing "query ident"
    (let [q (:query (meta sut/DeptDashboard))]
      (is (vector? q))
      (is (some #{:loading} q))
      (is (some #(and (map? %) (contains? % :available-units)) q)))
    (is (= :dept-dashboard/root (:ident (meta sut/DeptDashboard))))))
