(ns com.ozimos.workforce.frontend.ui.pages.create-org-test
  (:require
   [cljs.test :refer [deftest is testing]]
   [clojure.string :as str]
   [com.fulcrologic.fulcro.algorithms.denormalize :as denorm]
   [com.fulcrologic.fulcro.application :as app]
   [com.ozimos.workforce.frontend.ui.pages.create-org :as sut]
...
(deftest render-states
  (testing "initial form render"
    (let [hiccup (sut/CreateOrg (base-props {}))
  ...
  (testing "error message render"
    (let [html (rs/render (sut/CreateOrg (base-props {:error-msg "Name taken"})))]
      (is (str/includes? html "Name taken"))))
  (testing "loading button state"
    (let [html (rs/render (sut/CreateOrg (base-props {:loading true})))]
      (is (str/includes? html "Creating..."))))
  (testing "success state render"
    (let [html (rs/render (sut/CreateOrg (base-props {:success true :org-name "Acme Global"})))]
...
(deftest form-events
  (testing "input and submit events are pure data"
    (let [hiccup (sut/CreateOrg (base-props {}))
          input-ev (find-event-in-hiccup hiccup #(= (first %) :com.ozimos.workforce.frontend.ui.pages.create-org/set-name))
          submit-ev (find-event-in-hiccup hiccup #(= (first %) :com.ozimos.workforce.frontend.ui.pages.create-org/submit))]
      (is (= [:com.ozimos.workforce.frontend.ui.pages.create-org/set-name] input-ev))
      (is (= [:com.ozimos.workforce.frontend.ui.pages.create-org/submit] submit-ev)))))
