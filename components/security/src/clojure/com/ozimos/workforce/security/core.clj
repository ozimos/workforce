(ns com.ozimos.workforce.security.core
  (:require
...
(defn build-application-context
  ...
  (let [ctx (AnnotationConfigApplicationContext.)
    ...
    (.register ctx (into-array Class [(Class/forName "com.ozimos.workforce.security.SecurityConfig")]))
    (.refresh ctx)
    ctx))
...
(defmethod ig/init-key :security/app-context [_ {:keys [jwt-decoder rama user-store]}]
  (let [decoder (:decoder jwt-decoder)
        ...
        find-user-fn (fn [username]
                       ((requiring-resolve 'com.ozimos.workforce.user.interface/find-by-username) user-deps username))
